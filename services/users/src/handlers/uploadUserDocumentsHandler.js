...
const handler = async (event) => {
    try {
        ...
        for (const file of parts) {
            ...
            const id = uuidv4()
            const fileKey = `user-Documents/${userId}/${id}.${fileExtension}`;
        ...
        await db.collection('users').updateOne({ _id: userId }, { $push: { "identityVerification.documents": { $each: uploadedFiles } } });
